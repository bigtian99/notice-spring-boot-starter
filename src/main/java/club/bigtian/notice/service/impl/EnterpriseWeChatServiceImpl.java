...
public class EnterpriseWeChatServiceImpl implements INoticeService {
    ...
    public void sendHandledMessage(TExceptionInfo info) {
        ...
        String contents = StrUtil.format("## <font color=\"green\">{}</font>\n\n" +
                        ...
                        "> 导致原因：\r\n{}\n\n" +
                        "> 异常堆栈：[查看详情]({}) \n\n"
                , config.getSuccessTitle(), config.getProjectName(), info.getId(), info.getHandledMan(), users, DateUtil.format(info.getHandledTime(), "yyyy-MM-dd HH:mm:ss"), config.getEnvs().get(activeProfile),info.getCause(), urlPrefix + info.getId());
        sendNotice(contents);
    }
